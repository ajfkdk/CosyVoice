...
class CosyVoice3Demo:
    def __init__(self):
        ...
        print(f"[OK] 模型加载成功: {self.model_dir}")
    ...
    def play_15_audiobook_tags(self):
        """玩法15: 说书场景 - 副语言标记测试"""
        print("\n[*] 玩法15: 说书场景 - 副语言标记")
        text = 'You are a helpful assistant.<|endofprompt|>[breath]陈小姐听到这话，突然倒吸一口冷气。[breath]她的眼泪止不住地流下来，[sigh]长长地叹了口气。'
        ref_audio = r'C:\Users\pc\PycharmProjects\TOOL\Pictory\output_audio_20_30.mp3'

        for i, j in enumerate(self.cosyvoice.inference_cross_lingual(
            text, ref_audio, stream=False
        )):
            output = f'output/15_audiobook_tags_{i}.wav'
            torchaudio.save(output, j['tts_speech'], self.cosyvoice.sample_rate)
            print(f"[OK] 生成: {output}")

    def play_16_audiobook_emotions(self):
        """玩法16: 说书场景 - 多情感混合"""
        print("\n[*] 玩法16: 说书场景 - 多情感混合")
        text = '陈小姐这回真哭了。这一段情节其实大家都知道个大概，但是现在让陈小姐亲口说出来，还带有如此多的细节，真让人感慨不已。'
        instruct = 'You are a helpful assistant. 请用悲伤但坚定的语气说这段话，前半段悲伤，后半段逐渐平静。<|endofprompt|>'
        ref_audio = r'C:\Users\pc\PycharmProjects\TOOL\Pictory\output_audio_20_30.mp3'

        for i, j in enumerate(self.cosyvoice.inference_instruct2(
            text, instruct, ref_audio, stream=False
        )):
            output = f'output/16_audiobook_emotions_{i}.wav'
            torchaudio.save(output, j['tts_speech'], self.cosyvoice.sample_rate)
            print(f"[OK] 生成: {output}")
...
def main():
    ...
    # 说书专用测试
    # demo.play_15_audiobook_tags()    # 副语言标记
    demo.play_16_audiobook_emotions() # 多情感混合
